...
export function buildAuthHeader(email, token) {
...
export function resolveTransitionId(transitions, name) {
...
export function describeExecution(plan) {
...
export class JiraClient {
    ...
    async getTransitions(issueKey) {
        const data = (await this.req("GET", `/rest/api/3/issue/${issueKey}/transitions`));
    ...
    async assign(issueKey, accountId) {
...
export async function executeSyncPlan(plan, client) {
    const done = [];
    const skipped = [];
    if (plan.noop || !plan.issueKey) {
    ...
    if (plan.transition) {
        ...
        if (id) {
            ...
            skipped.push(`transition "${plan.transition}" not available (have: ${transitions
                .map((t) => t.name)
                .join(", ")})`);
        }
    }
