 ...
 */
export function applyDecision(manifest, input) {
    ...
    if (chunk.revisionId !== input.revisionId) {
        throw new Error(`prwalk: stale decision for ${input.stableId} (expected revision ${chunk.revisionId}, got ${input.revisionId}). Reload the review.`);
    }
    const ts = nowIso();
