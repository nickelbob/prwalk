...
import { makeAnchor } from "./identity.js";
/**
 ...
 */
export function inferRisk(path, patch, changeType) {
    if (changeType === "rename" || changeType === "mode")
        return 1;
    ...
    const allTrivial = changed.length > 0 &&
        changed.every((l) => l === "" ||
            l.startsWith("import ") ||
            l.startsWith("export {") ||
            l.startsWith("//") ||
    ...
    if (allTrivial)
        return 1;
    if (LOW_RISK_EXT.has(ext) || isTest)
        return 2;
    return 3;
}
function hunkToCandidate(file, hunk) {
...
function fileToSyntheticCandidate(file) {
    const path = file.newPath ?? file.oldPath ?? "unknown";
    const label = file.changeType === "binary"
