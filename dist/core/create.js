 ...
 */
export async function createReview(cwd, opts) {
    const warnings = [];
    ...
    if (!base) {
        throw new Error(`prwalk: no common ancestor between ${opts.base} and ${branch}; pick a valid base.`);
    }
    const dirty = await git.uncommittedCount(cwd);
    if (dirty > 0) {
        warnings.push(`${dirty} uncommitted change(s) are NOT included; the review reflects committed code only. Commit first for an accurate review.`);
    }
    ...
    const issueKey = opts.issue?.trim() ||
    ...
    const meta = {
        ...
        tracker: issueKey ? config.tracker ?? "jira" : null,
    };
