...
export function inScope(chunk, level) {
    if (level === null)
        return true; // no level chosen yet → everything is in scope
    return chunk.risk >= level;
}
/** True once the reviewer has explicitly decided a chunk (sticky across level
 * changes). */
export function isExplicit(chunk) {
    return ((chunk.decision.state === "accepted" || chunk.decision.state === "rejected") &&
 ...
 */
export function applyReviewLevel(manifest, level) {
    const ts = nowIso();
...
export function riskDistribution(manifest) {
...
//# sourceMappingURL=reviewLevel.js.map