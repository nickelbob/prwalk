...
export async function cmdStatus(cwd, branch, opts) {
    ...
    const lines = [];
    lines.push(`${report.branch} — round ${report.round} — ${report.status.toUpperCase().replace("_", " ")} ` +
        `(${report.counts.rejected} rejected, ${report.counts.pending} pending, ${report.counts.accepted} accepted)`);
    if (report.reviewLevel !== null) {
        lines.push(`  review level: ${report.reviewLevel}+ · ${report.autoAccepted} chunk(s) auto-accepted below level`);
    }
    else {
