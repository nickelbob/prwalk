...
export async function cmdReview(
  ...
  try {
    ...
    process.stderr.write(
      `prwalk: could not fetch from '${remote}' (${(e as Error).message}). ` +
  ...
  if (!branch) {
    ...
    if (matches.length === 0) {
      process.stderr.write(
        `prwalk: no branch found for ${issueKey} on '${remote}'. ` +
    ...
    if (matches.length > 1) {
      process.stderr.write(
        `prwalk: multiple branches match ${issueKey}: ${matches
          .map((m) => m.local)
  ...
  if (dirty > 0) {
    process.stderr.write(
      `prwalk: ${dirty} uncommitted change(s) in the working tree — checkout may fail or stash is needed.\n`,
    );
  }
  try {
    ...
    process.stderr.write(
      `prwalk: could not check out '${branch}' (${(e as Error).message}). ` +
